...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        ModuleVersion = '0.0.1'
        ModuleWebsite = 'https://github.com/justin-p/ModuleBuildToolsTemp'
        ModuleCopyright = "(c) $((get-date).Year.ToString()) Justin Perdok. All rights reserved."
        ModuleLicenseURI = 'https://github.com/justin-p/ModuleBuildToolsTemp/LICENSE.md'
        ModuleTags = 'ModuleBuildToolsTemp' -split ','
        ...
        # You should NOT enter this key here but rather manually enter it in the ModuleBuildToolsTemp.buildenvironment.json file with: set-buildenvironment -NugetAPIKey '<key>'
